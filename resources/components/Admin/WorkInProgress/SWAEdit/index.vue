...
const props = defineProps({
  SWAId: { type: String, required: true },
  InputInspection: { type: Array, required: true },
  FileIdWP: { type: String, required: true },
  FileInputs: { type: Array, required: true },
});
  ...
  try {
    openFileUploader({
      ...
      onConfirm: async (uploadedFile) => {
        ...
        const index = fileInputs.value.findIndex((f) => f.key === key);

        if (index !== -1) {
          fileInputs.value[index].fileinput = uploadedFile;
        } else {
...
const saveSWA = async () => {
  try {
    ...
    const response = await apiClient.post(
      `/work-in-progress/swa/${props.SWAId}/update`,
      formData,
      {
        headers: {
...
watch(
  ...
  async (val) => {
    if (val) {
      inspectionForm.value = props.InputInspection;
      fileInputs.value = await Promise.all(
        props.FileInputs.map(async (f) => {
          const fileId = f.FileId;
          const pdfUrl = `/storage/uploads/${fileId}.pdf`;

          const response = await fetch(pdfUrl);
          const blob = await response.blob();
          const file = new File([blob], `${fileId}.pdf`, {
            type: "application/pdf",
          });
